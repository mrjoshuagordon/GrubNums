	...
	*/
	...
	$time = date("Y-m-d h:i:sa", time()) ; 
		...
		mysql_query("INSERT INTO `grubs` (`user_id`, `image`, `name`, `time`) VALUES ('$user_id' , '$upload_name' , '$name', '$time' )");	 
		 ...
		 echo 'Image Uploaded! <br> <a href=./grubedit.php?image='.$upload_name.' target="blank"><img src="'.$display_name.'"> </a> <br>';
		 echo '<a href=./grubedit.php?image='.$upload_name.' target="blank">'. $file_name. '</a>';
		 
		 $image_id = image_id_from_imagename($upload_name);						
		mysql_query("INSERT INTO `grubs_ratings` (`grub_id`, `user_id`,`rating`) VALUES ('$image_id', '$user_id' , '0')");
...
function add_image($image_id, $title, $location, $description, $price, $active){
	...
	$query = mysql_query("SELECT COUNT(`grub_id`) from `image_data` WHERE `grub_id` = '$image_id' ");
		...
		mysql_query("UPDATE `image_data` SET `title` = '$title' , `location`= '$location' , `description` = '$description', `price` = '$price', `time` = '$time', `active` = '$active' WHERE `grub_id` = '$image_id' ");
					
	
		} else{
				
		mysql_query("INSERT INTO `image_data` (`grub_id`, `title`, `location`, `description`, `price`, `time`, `active`) VALUES ('$image_id' , '$title' , '$location','$description', '$price', '$time', '$active' )");
